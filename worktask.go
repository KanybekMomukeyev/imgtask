...
func uploadToPythonCudaServerAsync(imagePath string, docModelID uint64, docName string) error {
	...
	// peopleToReturn := new(ComplexResponseModel)
	// peopleToReturn.CuttedImages = cuttedImages
	// peopleToReturn.FolderModel = folderModel
	// fmt.Printf("\n\n ComplexResponseModel: %v", peopleToReturn)
	// // return c.JSON(http.StatusOK, peopleToReturn)
